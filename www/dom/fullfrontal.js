async function handleCreateProject(event) {
    ...
    if (!response.ok) {
        currentTarget.insertAdjacentHTML('beforebegin', `<p class="error">${responseData.message}</p>`);
...
function handleDeleteteProject(event) {
    event.preventDefault();
...
async function handleDeployProject(event) {
    event.preventDefault();
    const { value: projectID } = event.currentTarget;
    const response = await fetch(`/functions/fullfrontal/projects/deployments`, {
        method: 'POST',
...
async function main() {
    const createFullfrontalProjectForm = document.getElementById('create-fullfrontal-project');
    ...
    if (projects.length > 0 && projectsListingElement !== null) {
        projects.map((project) => {
            ...
            const projectDeleteButton = li.querySelector(`[value="delete-project"]`);
            if (projectDeleteButton) {
                projectDeleteButton.onclick = handleDeleteteProject;
            }
            const projectDeployButton = li.querySelector(`[name="deploy-project"]`);
...
export { main };
//# sourceMappingURL=fullfrontal.js.map
